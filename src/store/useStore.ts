...
export function useFilteredControls() {
  return useStore(useShallow((state) => {
    ...
    const filtered = controls.filter((c) => {
      ...
      if (filters.ismDomain !== 'All' && c.ism_domain !== filters.ismDomain) return false;
      ...
      if (filters.search) {
        ...
        if (
          ...
          !c.ism_domain.toLowerCase().includes(q) &&
    ...
    if (filters.framework === 'ISM') {
      return [...filtered].sort((a, b) =>
        a.ism_domain.localeCompare(b.ism_domain) || a.control_id.localeCompare(b.control_id)
      );
    }
